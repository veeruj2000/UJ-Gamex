let main = {
  ...
  methods: {
    gamesetup: function() {
      ...
      $('.gamecell').each(function(index) {
        let x = index % 8; // Column index (0-7)
        let y = Math.floor(index / 8); // Row index (0-7)
    
        if ((x + y) % 2 === 0) {
          $(this).css('background', '#f0d9b5'); // Light square color
        } else {
          $(this).css('background', '#b58863'); // Dark square color
        }
      });
  ...
  if (numOfKings == 1) {
    setTimeout(() => {
        let winnerText = main.variables.turn == 'w' ? 'Black Wins !!' : 'White Wins !!';
