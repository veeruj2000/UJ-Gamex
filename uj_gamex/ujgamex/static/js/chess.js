  ...
  methods: {
    ...
    endturn: function(){
      ...
      if (main.variables.turn == 'w') {
        ...
        $('#turn').html("It's Blacks Turn").css("color", "black");
    ...
    if (main.variables.selectedpiece == '' && target.name.slice(0,1) == main.variables.turn) { // show options
      ...
      if (selectedpiece.name == 'w_king' || selectedpiece.name == 'b_king'){
        ...
        if (t0 && t2 && t4 && t6) { // castle w_king
          movePiece('w_king', 'w_rook2', '5_1', '7_1', '8_1', '6_1');
          main.methods.endturn();
          setTimeout(checkmate, 500);
          setTimeout(aiMove, 1000); // AI moves after player
          
        } else if (t1 && t2 && t3 && t5) { // castle b_king
          movePiece('b_king', 'b_rook2', '5_8', '7_8', '8_8', '6_8');
          main.methods.endturn();
          setTimeout(checkmate, 500);
          setTimeout(aiMove, 1000); // AI moves after player
      ...
      if (selectedpiece.id != target.id && main.variables.highlighted.indexOf(target.id) != (-1)) { // capture valid piece
...
// ✅ AI Move Function
function aiMove() {
    if (main.variables.turn === 'b') { // AI plays as black
        let bestMove = getAIMoves();
        if (bestMove) {
            main.methods.move(bestMove);
            main.methods.endturn();
        }
    }
}
...
function getAIMoves() {
    ...
    Object.keys(main.variables.pieces).forEach(piece => {
        if (piece.startsWith('b_')) { // AI controls black pieces
            let moves = main.methods.getValidMoves(piece);
            if (moves.length > 0) {
    ...
    if (possibleMoves.length > 0) {
        let selected = possibleMoves[Math.floor(Math.random() * possibleMoves.length)];
        let moveTo = selected.moves[Math.floor(Math.random() * selected.moves.length)];
        return { name: selected.piece, id: moveTo };
    }
    return null;
}

// ✅ Helper Function for Castling Moves
function movePiece(king, rook, k_position, k_target, r_position, r_target) {
    main.variables.pieces[king].position = k_target;
    main.variables.pieces[king].moved = true;
    $('#' + k_position).html('');
    $('#' + k_position).attr('chess', 'null');
    $('#' + k_target).html(main.variables.pieces[king].img);
    $('#' + k_target).attr('chess', king);

    main.variables.pieces[rook].position = r_target;
    main.variables.pieces[rook].moved = true;
    $('#' + r_position).html('');
    $('#' + r_position).attr('chess', 'null');
    $('#' + r_target).html(main.variables.pieces[rook].img);
    $('#' + r_target).attr('chess', rook);
}
...
function checkmate() {
  ...
  // Find King Positions
  Object.keys(main.variables.pieces).forEach(piece => {
      if (piece === "w_king") kingPositions.white = main.variables.pieces[piece].position;
      if (piece === "b_king") kingPositions.black = main.variables.pieces[piece].position;
  });

  // If either king is missing, declare checkmate
  if (!kingPositions.white) {
      alert("Checkmate! Black Wins! 🏆");
      saveGameResult("loss");  // Save result for the player
      resetGame();
      return;
  } else if (!kingPositions.black) {
  ...
  let isWhiteInCheck = isKingInCheck("w_king");
  let isBlackInCheck = isKingInCheck("b_king");

  // If a king is in check and has no legal moves, it's checkmate
  if (isWhiteInCheck && !hasLegalMoves("w_king")) {
      alert("Checkmate! Black Wins! 🏆");
      saveGameResult("loss");
      resetGame();
  } else if (isBlackInCheck && !hasLegalMoves("b_king")) {
      alert("Checkmate! White Wins! 🏆");
      saveGameResult("win");
      resetGame();
  }
...
// 2️⃣ Save game result function
function saveGameResult(result) {
  fetch("/update-chess-score/", {
      method: "POST",
...
// 3️⃣ Reset function
function resetGame() {
  // Reset game variables, board, and turn
  console.log("Game has been reset.");
}

// 4️⃣ CSRF Token function (helper)
